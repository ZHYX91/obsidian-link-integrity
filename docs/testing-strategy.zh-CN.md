...
2026-08-08 增加了全孤立结果的侧栏查询刷新门禁，覆盖每次索引通知后必经的文件候选分类、孤立投影、诊断投影和结果物化。最终验证在精确 runtime 下，10k 个孤立文件连续 6 次完整投影共 192.7 ms，50k 个孤立文件连续 3 次共 494.6 ms；门禁分别采用 2,000 ms 和 8,000 ms 的保守上限。这说明当前实现没有后台轮询，但打开侧栏后每批真实索引变化仍会产生一次 O(文件数 + occurrence 数) 的派生查询；这些合成数字用于防回归，不替代真实 Vault 的交互掉帧与最长主线程任务测量。
