...
自动 UI 测试应覆盖两业务页签、三设置页签、无固定侧栏标题/刷新/设置行、首次建立索引、异常时上下文重建、设置页常驻重建入口、搜索、等宽视图切换、分组主按钮的立即切换、目标/来源文件/来源文件夹原生分组下拉框、按视图显示或隐藏的情境化原生排序、列表与树的固定排序契约、来源树计数和折叠分支惰性 DOM、临时格式筛选、预期孤立高级显示、单文件预期孤立操作、文件夹精确/递归预期孤立规则及撤销、低置信度标记，以及固定 100 条分页的首尾页可达性。
