...
自动 UI 测试应覆盖两业务页签、三设置页签、搜索、排序、列表/树或分组视图、临时格式筛选、预期孤立高级显示、低置信度标记，以及固定 200 条分页的首尾页可达性。
...
当前实现已经运行本地合成 10k 和显式 50k 模式。最新显式 50k 运行在记录的本地 runtime 上以 733.3 ms 完成建图和孤立投影，包含 guardrail 的完整基准测试为 1.121 s。100k 尚未验证，不能从 50k 结果推断。
