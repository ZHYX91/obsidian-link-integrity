...
On 2026-08-08, the guardrail gained an all-isolated sidebar-query refresh case covering the file-candidate classification, isolation projection, diagnostic projection, and result materialization performed after each index notification. In final verification on the exact runtime, six complete projections of 10k isolated files took 192.7 ms total, while three complete projections of 50k isolated files took 494.6 ms total; the gates use conservative ceilings of 2,000 ms and 8,000 ms respectively. This confirms that the implementation has no background polling, but an open sidebar still performs one O(file count + occurrence count) derived query per real index-change batch. These synthetic figures detect regressions and do not replace real-Vault interaction-jank or longest-main-thread-task measurement.
