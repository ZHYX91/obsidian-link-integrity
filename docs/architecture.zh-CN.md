...
- `src/scheduling/`：不依赖业务层的浏览器任务让步、共享时间预算和可分片排序。
...
create、delete 和 rename 会重新取得文件 registry，并比较新旧 lookup keys。普通修改只读取本批次的文件记录，通过现有 registry 与本批次覆盖判断来源是否存在，不复制全库路径集合。适配器在文件记录中保存标题文本与层级、有序标题、块标识和脚注标识的目标指纹；缓存未就绪时指纹为 null。只有两个已就绪指纹相同时才跳过普通修改的引用方传播；未知指纹、命名空间变化和全局元数据纠偏保持保守重验证。来源自身仍重建快照，以更新链接位置、内容与修改时间。Vault modify 不能替代后续 Metadata Cache changed 事件。

全量构建与增量更新共用调度器，默认在累计 128 个工作单元或约 8 ms 后通过浏览器任务让出执行机会。工作单元包括文件记录、链接解析、快照规范化、校验和 reducer 步骤，多个异步读取共享预算。该预算不是硬实时上限，单次宿主 API、JSON 解析或容器复制仍需用宿主性能记录检查。完整来源快照经同一可迭代 reducer 处理；同步调用直接消费步骤，运行时只在未发布 staging 上分片消费。全量构建保持单一 staging reducer，避免并发来源校验互相越过。

增量 staging 使用分桶写时复制映射，与已发布索引共享未变化的数据；触及的桶及可变集合在写入前取得独立所有权。每批次完成校验和归约后统一交换索引引用并递增发布代次。让步期间的新事件、索引版本变化或生命周期变化使旧 staging 失效；新事件与尚未提交的原批次重新合并。失败或取消不能改变已发布的索引，外部持有的旧索引也不会被后续有效增量原地改写。宿主级 `resolved` 在首次等待超时后仍触发一次完整校正，并刷新目标指纹：仅靠断链或 pending 标记无法可靠排除已解析结果重新定向，因此这条路径保留完整范围并使用同一分片调度。之后的普通 resolved 信号仍被忽略。

快照替换先验证规范化输入，再比较包含位置的完整语义；增量 staging 不重复执行一次批次规范化再执行一次发布规范化。普通快照替换沿当前贡献 policy 局部更新，只有 policy 或显式贡献范围变化才重新求值整张图。全量 staging 继承协调器当前的 policy，并在发布前再次同步规则变化。
...
每次增量发布提供变化来源、文件记录和图端点清单。查询层为非活动页签累积清单，按来源替换无效链接，按变化文件和旧、新端点更新孤立状态；排序后的局部结果与未变化结果线性合并，不重新排序整个诊断集。结果未变化时保留原数组身份。设置变化和完整 baseline 发布仍使相关缓存完整失效，运行时投影和排序经过调度后发布，过期投影不得覆盖新状态。

侧栏缓存筛选、排序和视图模型，进度数字变化只更新状态区。固定筛选和排序下，只排序内容变化的行并与保留结果合并；按组数量排序时，由于组大小可能影响其他组，允许分片重排全部可见结果。完整统计和目录计数也在准备阶段分片计算，分页呈现只读取准备好的计数。当前页未变化时保留结果区；变化时按结果 ID 或文件路径复用内容未变的行，位置、语言或行角色变化替换对应行及事件参数。行缓存限制在当前呈现结果内。不可见页签停止请求新的展示计算，恢复可见后读取最新索引；持续索引维护不依赖侧栏可见性。异步搜索和视图请求带代次，过期请求停止计算或丢弃输出；关闭视图断开可见性观察与查询订阅。
