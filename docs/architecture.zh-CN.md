...
插件按“启动时扫描”设置决定是否在工作区就绪后主动执行全量重建。禁用时，协调器仍立即开始捕获 Vault 事件，但不会用局部事件建立不完整 baseline；恢复或首次打开侧栏时自动触发完整重建，设置页和未扫描空状态同时提供可发现的手动“建立 / 重建索引”操作。baseline 成功后，增量控制器在当前会话持续同步，因此日常变化不需要手动刷新。若将来性能证据要求跨重启缓存，也只能把它视为可验证缓存，不能绕过当前 Vault 的重新解析。
