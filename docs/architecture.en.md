...
- `src/scheduling/`: browser task yields, shared time budgets, and sliced sorting without business-layer dependencies.
...
The adapter builds a complete snapshot for every supported explicit source. Markdown and Frontmatter use the Metadata Cache with text extraction as a fallback where needed; Canvas reads explicit file nodes, background files, and internal links in text; Bases parses YAML first and extracts statically identifiable explicit references only from formula scalars in `filters`, `formulas`, custom `summaries`, and view `filters`. View names, `properties` display configuration, and ordinary strings inside formulas do not create edges, and dynamic Bases query results never enter the graph model; malformed Bases YAML fails closed for that batch. A link() first argument must be a complete string literal; concatenations and other dynamic expressions do not create edges. Positions are exact only when the decoded formula appears unchanged in its YAML scalar; otherwise the reference is retained without a claimed line/column. Repeated references are preserved in either case. Invalid Canvas JSON fails closed in the same way: the batch does not replace file metadata or snapshots, and an existing last-known-good index remains visible as stale instead of turning that source into a high-confidence isolated result.

The Markdown fallback preserves UTF-16 source offsets while masking fenced and indented code, inline code, Obsidian comments, and YAML comment ranges inside BOM-aware `---`/`...` frontmatter. Fenced code uses a line-state machine that handles LF/CRLF, empty and adjacent blocks, both fence characters and variable lengths, and unclosed fences without newline normalization that would shift navigation offsets. It retains explicit links in frontmatter values and ordinary Markdown text. Canvas text nodes use the same fallback, so transient startup parsing and Canvas diagnostics share one false-positive boundary.
...
Create, delete, and rename events refresh the file registry and compare old and new lookup keys. Ordinary modifications read only the batch's file records and check source existence through the existing registry plus batch overrides without copying every Vault path. The adapter stores a target fingerprint of heading text, levels and order, block IDs, and footnote IDs in each file record; unavailable metadata produces null. Only matching ready fingerprints suppress referer invalidation for ordinary modifications. Unknown fingerprints, namespace changes, and global metadata correction retain conservative revalidation. The source itself still rebuilds to update link positions, content, and modification time. Vault modify never replaces the subsequent Metadata Cache changed event.

Full construction and incremental updates share a scheduler that yields through a browser task after 128 work units or approximately 8 ms by default. Work units include file records, link resolution, snapshot normalization, validation, and reducer steps; concurrent reads share the budget. This is not a hard real-time limit: individual host APIs, JSON parsing, and container copies still require host profiling. Complete source snapshots pass through one iterable reducer. Synchronous callers consume its steps directly; the runtime consumes them in slices only on unpublished staging. Full construction keeps one staging reducer so concurrent source validation cannot race past each other.

Incremental staging uses bucketed copy-on-write maps and shares unchanged data with the published index. Touched buckets and mutable collections acquire independent ownership before mutation. Once validation and reduction finish, a batch swaps the index reference and advances the publication generation. New events, index-version changes, or lifecycle changes during a yield invalidate obsolete staging; new events coalesce with the original uncommitted batch. Failure and cancellation cannot change the published index, and retained old indexes are not mutated by subsequent valid updates. A host-wide `resolved` after the initial timeout still triggers one complete correction and refreshes target fingerprints. Broken or pending markers alone cannot reliably exclude retargeting of previously resolved links, so this path retains complete coverage through the same sliced scheduler. Later ordinary resolved signals remain ignored.

Snapshot replacement validates normalized input and then compares complete semantics including positions. Incremental staging avoids normalizing once for batch prevalidation and again for publication. Ordinary replacement evaluates the active contribution policy locally; only policy or explicit contribution-scope changes reevaluate the whole graph. Full-rebuild staging inherits the coordinator's current policy and synchronizes rule changes again before publication.
...
Each incremental publication reports changed sources, file records, and graph endpoints. Queries accumulate changes for inactive tabs, replace broken diagnostics by source, and update isolation for changed files and old/new endpoints. Ordered local results merge linearly with unaffected results without sorting all diagnostics again. Unchanged results preserve array identity. Settings changes and complete baseline publication still invalidate relevant projections in full; runtime projection and sorting are scheduled before publication, and obsolete projections cannot overwrite new state.

The sidebar caches filtering, sorting, and view models; progress-number changes update only the status area. With unchanged filters and ordering, only changed rows are sorted and merged with retained results. Group-count ordering may re-sort all visible results in slices because changed group sizes can affect other groups. Complete statistics and folder counts are also prepared in slices; page materialization reads prepared counts. An unchanged current page retains its result area. Changed pages reuse unchanged rows by occurrence ID or file path, while position, language, or row-role changes replace the corresponding row and event payload. Row caches are limited to currently rendered results. Invisible tabs stop requesting new presentation work and catch up from the current index when visible again; continuous indexing does not depend on sidebar visibility. Asynchronous searches and view requests carry generations so obsolete requests stop computing or discard their output. Closing a view disconnects its visibility observer and query subscription.

## Persistence and recovery

The plugin does not persist `LinkIndex`, edges, or diagnostic projections. `data.json` stores only settings, rules, and interface preferences after schema validation, migration, and normalization. Expected-isolation rules share one validation contract across preview, save, and load: a naming condition that cannot be retained safely is never silently dropped in a way that broadens a match, and an existing invalid rule is disabled as a unit. Advanced regular expressions and globs use a Thompson state-set matcher: each state is visited at most once per input code point, without executing the complete user expression in the native backtracking engine. Regex source is limited to 512 characters, repetition counts to 512, and compiled patterns to 1024 states. Backreferences, lookaround, special groups other than noncapturing groups, and flags other than i/u are unsupported. This prevents both a stale namespace from a previous run becoming authoritative and an incompatible setting silently changing meaning.
...
There is currently no derived-graph persistence, external-URL network checking, automatic deletion, or bulk repair. Architecture tests do not establish the required Obsidian 1.12.7/current 1.13.x desktop-host matrix, live-event paths, or Android-emulator behavior; desktop-host acceptance is always separate, and a candidate with `isDesktopOnly: false` also requires current Android-emulator evidence. Android physical devices and iOS are out of scope; historical physical-device evidence is supplemental only and never a release gate.
